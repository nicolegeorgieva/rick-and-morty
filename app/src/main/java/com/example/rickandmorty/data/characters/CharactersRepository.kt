...
class CharactersRepository @Inject constructor(
  ...
  suspend fun fetchCharacters(page: Int?): Either<Throwable, Characters> {
    return charactersDataSource.fetchCharacters(page).map { charactersDto ->
      Characters(
  ...
  @OptIn(ExperimentalTime::class)
  suspend fun fetchCharacter(id: String): Either<Throwable, Character> {
    return charactersDataSource.fetchCharacter(id).map { characterDto ->
      Character(
        id = characterDto.id,
        name = characterDto.name,
        status = characterDto.status,
